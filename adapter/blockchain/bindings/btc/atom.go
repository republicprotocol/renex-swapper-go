...
func Initiate(conn btc.Conn, myKey keystore.BitcoinKey, participantAddress string, value int64, hash []byte, lockTime int64) (bitcoinData, error) {
...
func Redeem(conn btc.Conn, myKey keystore.BitcoinKey, contract, contractTxBytes []byte, secret [32]byte) (redeemResult, error) {
	...
	if recipientAddr.EncodeAddress() != myKey.AddressString {
...
func Refund(conn btc.Conn, myKey keystore.BitcoinKey, contract, contractTxBytes []byte) error {
...
func buildContract(conn btc.Conn, args *contractArgs, key keystore.BitcoinKey) (*builtContract, error) {
...
func buildRefund(conn btc.Conn, key keystore.BitcoinKey, contract []byte, contractTx *wire.MsgTx) (
	...
	refundOutScript, err := txscript.PayToAddrScript(key.Address)
	if err != nil {
		return nil, err
	}
	...
	if refundAddr.EncodeAddress() != key.AddressString {
...
func createSig(conn btc.Conn, tx *wire.MsgTx, idx int, pkScript []byte, key keystore.BitcoinKey) (sig, pubkey []byte, err error) {
	sig, err = txscript.RawTxInSignature(tx, idx, pkScript, txscript.SigHashAll, key.PrivateKey)
	if err != nil {
		return nil, nil, err
	}
	return sig, key.PrivateKey.PubKey().SerializeUncompressed(), nil
}
