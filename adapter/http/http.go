...
func (adapter *adapter) WhoAmI(challenge string) (WhoAmISigned, error) {
	...
	ethKey := adapter.keystr.GetKey(tokens.TokenETH).(keystore.EthereumKey)
...
func (adapter *adapter) PostOrder(order PostOrder) (PostOrder, error) {
	...
	key := adapter.keystr.GetKey(tokens.TokenETH).(keystore.EthereumKey)
...
func (adapter *adapter) GetBalances() (Balances, error) {
	ethBal, err := ethereumBalance(
		adapter.config,
		adapter.keystr.GetKey(tokens.TokenETH).(keystore.EthereumKey),
	)
	if err != nil {
		return Balances{}, err
	}
	btcBal, err := bitcoinBalance(
		adapter.config,
		adapter.keystr.GetKey(tokens.TokenBTC).(keystore.BitcoinKey),
	)
	if err != nil {
		return Balances{}, err
	}
