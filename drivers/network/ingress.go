...
func NewIngress(hostAddress string) Network {
	return &ingress{
...
func (ingress *ingress) RecieveOwnerAddress(orderID order.ID, waitTill int64) ([]byte, error) {
	for {
		...
		if time.Now().Unix() > waitTill {
			return nil, fmt.Errorf("Recieve Address Timedout")
		}
		time.Sleep(10 * time.Second)
	}
}

func (ingress *ingress) RecieveSwapDetails(orderID order.ID, waitTill int64) ([]byte, error) {
	for {
		...
		resp.Body.Close()
		if time.Now().Unix() > waitTill {
			return nil, fmt.Errorf("Recieve Swap Details Timedout")
		}
		time.Sleep(10 * time.Second)
	}
}
